...
describe('withRetry', () => {
  it('returns the first success without retrying', async () => {
    let calls = 0
    const result = await withRetry(async () => ++calls, { maxRetries: 3, baseDelayMs: 1, sleep: async () => {} })
  ...
  it('retries until success and reports each retry', async () => {
    ...
    const result = await withRetry(
      ...
      { maxRetries: 5, baseDelayMs: 1, sleep: async () => {}, onRetry: attempt => retries.push(attempt) },
    )
  ...
  it('waits with exponential backoff between attempts', async () => {
    ...
    await withRetry(
      ...
      { maxRetries: 3, baseDelayMs: 100, sleep: async (ms) => { delays.push(ms) } },
    )
