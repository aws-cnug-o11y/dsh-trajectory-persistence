...
on-disk artifact **read-only** and uploads byte-faithful zstd frame segments
plus a per-session `_manifest.json`. On another machine, the bundled
`sync-down` CLI restores the artifact from those segments, so a fresh machine
can list and `dsh resume` the session.
...
Everything else has defaults; the full field table lives in the
[Configuration Reference](/guide/configuration#sinks-s3).
...
### Watermark and writerId

Progress is tracked locally in a ship-state file and **authoritatively** in
the per-session `_manifest.json`: on first contact with a session the
manifest's `watermark` wins over the local offset, so a lost or stale state
file resumes instead of re-shipping. Segment keys are deterministic and
zero-padded (`00000000000000-00000000262144.jsonl.zstd`), so a crash between
segment put and manifest write simply re-puts the same key.
...
[`sinks.s3` reference](/guide/configuration#sinks-s3), and adds the
artifact-tailing fields (`root`, `pollIntervalMs`, `segmentBytes`,
`segmentMaxDelayMs`, `dormantAfterMs`, `writerId`) documented in the same
table. The push-mode fields (`batchSize`, `maxBufferedEvents`, `deadLetterDir`)
do not apply — durability comes from the official artifact itself, which the
shipper only tails.
...
2. **Machine B** — make sure dsh is **not** running against the local session
   ...
   ```sh
   dsh-trajectory-persistence sync-down --bucket my-bucket --region us-east-1
   ```
...
| | `mode: 'push'` (legacy, default) | `mode: 'ship'` |
|---|---|---|
| Data source | Live event stream (`session/event` firehose) | Official jsonl backend's on-disk artifact (read-only tail) |
| Artifact format | Self-contained JSONL parts (`{seqStart}-{seqEnd}.jsonl`, header line + events) | Byte-faithful zstd frame segments + `_manifest.json` watermark |
| Latency | Near real-time (flush at `batchSize` / `session/flush`) | Poll-driven (`pollIntervalMs`; segments at `segmentBytes` or after `segmentMaxDelayMs`) |
...
`mode: 'push'` remains the default and is fully supported, but it is
**legacy**: new deployments that want restorable sessions should use
`mode: 'ship'`. The two modes cannot run in the same sink instance — pick one
per `sinks.s3` config.
